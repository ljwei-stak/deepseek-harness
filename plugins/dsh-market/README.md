...
The Windows desktop runtime loads this plugin as a separate Web profile bundle beside Model Router + GALGame. It also carries Node.js and `pnpm@11.7.0`, so local mode can install market packages on a clean computer without a system Node toolchain. Plugins installed through the market remain dependencies of the user's Web profile and are not written into either bundled plugin directory.
