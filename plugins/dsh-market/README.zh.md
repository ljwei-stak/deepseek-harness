...
Windows 桌面本地运行时把本插件作为独立 Web profile bundle，与 Model Router + GALGame 并列加载。桌面端同时携带 Node.js 和 `pnpm@11.7.0`，因此全新电脑不需要系统预装 Node 工具链，也能在本地模式安装市场插件。市场安装的插件仍作为用户 Web profile 的依赖保存，不会写入任一内置插件目录。
