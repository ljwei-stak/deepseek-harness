...
- 项目更新：插件设置页检查固定的项目 Release 更新源。一键更新会在客户端过期时连同内置插件一起更新完整客户端，否则只更新插件；两个手动入口仍然保留。纯网页安装不能修改本机文件，因此会打开 Releases 页面。
...
仓库根目录的 `desktop/` 提供服务器端/本地运行模式切换和 Windows 打包配置。已构建的安装包与独立插件包通过 [Model Router + GALGame 0.4.9 Release](https://github.com/ljwei-stak/deepseek-harness/releases/tag/model-router-galgame-0.4.9) 发布；源码仓库只保留构建脚本和可复现的插件资源。
