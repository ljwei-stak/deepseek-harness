...
import {
  nodesToLines, partialToText, deriveStatus, routeFromNode, speakerFor, shouldRenderMarkdown, welcomeLine,
} from './transcript.mjs'
 ...
 */
export function GalView({ sessionId, useSession, useSessions, useInput, inputActions, attachmentApi, useScene, useHistory, useAssets, useFonts, useStore, useRouter, routerActions, openSession, actions, api }) {
  ...
  const send = useSend(inputActions, draft, setDraft, hasImages)
    ...
    <div className="gv-root" data-gal-view="" data-gal-mode={mode} data-taskbar={mode === 'game' ? (taskbarOpen ? 'open' : 'collapsed') : 'editor'} ref={rootRef}>
      ...
      {mode === 'game' && (
        <>
          ...
          >
            <AttachmentPicker inputActions={inputActions} inputState={inputState} attachmentApi={attachmentApi} draft={draft} setDraft={setDraft} />
            <textarea
