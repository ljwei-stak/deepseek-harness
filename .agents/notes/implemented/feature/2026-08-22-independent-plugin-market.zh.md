...
原生 Harness 用户可以在不安装 Model Router 的情况下安装、删除或更新市场。桌面用户通过同一安装包获得两个插件，但它们的运行时身份、源码目录、设置和更新归属保持独立。Profile 实体化会复制体积较小的市场依赖闭包，但可避免 ESM 解析依赖安装目录或开发源码目录。本地快照会增加仓库体积，并要求每次上游刷新同步更新源码、构建文件、来源记录、锁数据、测试和声明。便携 pnpm 会增加安装包体积。市场安装的代码会扩展可信计算基，因此必须由用户明确审查。
