...
- 价格单位为每百万 token；人民币和美元分开保存、分开绘图，不在没有汇率声明的情况下直接相加。
...
图表脚本使用统一场景 `100K` 输入、`10K` 输出和 `40%` 输入缓存命中率。对没有缓存读取价的模型，场景费用按 `100K` 普通输入计算；对有缓存读取价的模型，按 `60K` 普通输入加 `40K` 缓存读取计算。人民币和美元结果分别展示，图中的数值是估计费用，不是账单。
